...
# --- v1 challenger only: within-chunk normalization of absolute *_bb magnitudes.
# Divides every bb aggregate by the chunk's own median pot, flipping absolute
# big-blind magnitudes (2-11 sigma OOD on live, where pots run ~half benchmark)
# into scale-invariant relative units. Feature names and count are unchanged;
# only the values differ, so the effect can be measured as a single variable.
_BB_FEATURES: tuple[str, ...] = (
    "schema_amount_mean_bb",
    "schema_amount_std_bb",
    "schema_amount_q90_bb",
    "schema_amount_max_bb",
    "schema_pot_before_mean_bb",
    "schema_pot_after_mean_bb",
    "schema_pot_delta_mean_bb",
    "schema_pot_growth_bb",
    "schema_starting_stack_mean_bb",
    "schema_starting_stack_std_bb",
    "schema_starting_stack_iqr_bb",
)


def _normalize_bb_within_chunk(per_hand: list[dict[str, float]]) -> None:
    """Flip absolute *_bb features to within-chunk-relative (divide by chunk median pot).

    Every *_bb aggregate is scale-linear (mean/std/quantile/max of amounts), so
    dividing the per-hand aggregate equals normalizing the raw amounts. The chunk
    median pot shifts ~0.5x on live just like every other magnitude, so the ratio
    is invariant to the benchmark->live scale shift while bet/pot/stack
    relationships are preserved. Mutates ``per_hand`` in place.
    """
    pots = sorted(
        f["schema_pot_before_mean_bb"] for f in per_hand if f["schema_pot_before_mean_bb"] > 0.0
    )
    if pots:
        mid = len(pots) // 2
        ref = pots[mid] if len(pots) % 2 else 0.5 * (pots[mid - 1] + pots[mid])
    else:
        ref = 0.0
    ref = ref if ref > 1e-9 else 1.0
    for feats in per_hand:
        for key in _BB_FEATURES:
            feats[key] = feats[key] / ref
...
def chunk_features(chunk: list[dict[str, Any]]) -> dict[str, float]:
    ...
    _normalize_bb_within_chunk(per_hand)  # v1 challenger only: absolute *_bb -> within-chunk-relative
...
# --------------------------------------------------------------------------- #
# schema_rp_* : size-invariant self-redundancy of the chunk's bag of hands.
# Bots replay near-identical sequences; humans vary. All values are ratios /
# per-hand-normalized so 30-hand and 100-hand chunks are comparable (robust to
# the benchmark->live chunk-size shift, unlike absolute *_bb magnitudes).
# Pairwise work is capped at _RP_MAX_HANDS via deterministic stride subsampling.
# Named schema_* so the robust-feature allowlist admits them.
# --------------------------------------------------------------------------- #
...
def _rp_features(
    ...
    if n_all < 2:
        return {
            "schema_rp_pair_jaccard_mean": 0.0,
            "schema_rp_vendi_frac": 1.0,
            "schema_rp_exact_dup_frac_action": 0.0,
            "schema_rp_exact_dup_frac_rich": 0.0,
            "schema_rp_gzip_ratio": 1.0,
            "schema_rp_lz76_norm": 0.0,
            "schema_rp_entropy_rate": 0.0,
        }
    ...
    out["schema_rp_exact_dup_frac_action"] = 1.0 - _safe_div(len(set(action_signatures)), n_all)
    out["schema_rp_exact_dup_frac_rich"] = 1.0 - _safe_div(len(set(rich_signatures)), n_all)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            row_sums[i] += s
            row_sums[j] += s
    out["schema_rp_pair_jaccard_mean"] = _mean_or(sims, 0.0)
    ...
    if total_mass > 0:
        ...
        out["schema_rp_vendi_frac"] = _clamp01(math.exp(ent) / n)
    else:
        out["schema_rp_vendi_frac"] = 1.0
    ...
    out["schema_rp_gzip_ratio"] = whole / parts
    ...
    out["schema_rp_lz76_norm"] = _rp_lz76_norm(flat_str[:300])
    out["schema_rp_entropy_rate"] = _rp_entropy_rate(flat_actions[: 100 * 40])
    return out


def union_features(chunk: list[dict[str, Any]]) -> dict[str, float]:
    """Base chunk features plus the extended feature views (``e_`` prefixed to
    avoid key collisions). The trained artifact's ``feature_names`` selects the
    retained subset, so serving computes this union and keeps only those columns.
    """
