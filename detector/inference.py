...
class Poker44Model:
    ...
    @staticmethod
    def _within_batch_percentile(column: list[float]) -> list[float]:
        """Average (tie-aware) rank of a column, rescaled to (rank-0.5)/n."""
        ...
        out[order] = ranks
        return ((out - 0.5) / float(n)).tolist()

    def _blend_per_model(
        self, per_model: list[list[float]], n_rows: int
    ) -> list[float]:
        """Combine per-model score columns into a single column via a weighted mean."""
        ...
        columns = per_model
        if str(self.metadata.get("blend_mode", "")).strip() == "rank_vote":
            columns = [self._within_batch_percentile(column) for column in per_model]

        scores: list[float] = []
        for row_index in range(n_rows):
            value = sum(
                weight * model_scores[row_index]
                for weight, model_scores in zip(weights, columns)
            ) / total_weight
